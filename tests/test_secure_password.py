...
class HashPasswordTester(unittest.TestCase):
    ...
    def init_passwords(self) -> None:
        ...
        for i in range(2):
            ...
            pair = {"pass": password, "hash": self.secure.create_salted_hash(password)}
            self.passwords.append(pair)

    def test_verify_password(self) -> int:
        self.assertTrue(
            self.secure.verify_password(self.passwords[0]["pass"], self.passwords[0]["hash"])
        )
        self.assertFalse(
            self.secure.verify_password(self.passwords[0]["pass"], self.passwords[1]["hash"])
        )
...
class EncryptedPasswords(unittest.TestCase):
    ...
    def test_encrypt_password(self):
        self.assertTrue(isinstance(self.cipher_text, bytes), "cipher text in wrong format")
        self.assertNotEqual(self.cipher_text, self.password, "the password is not encrypted")

    def test_decrypt_password(self):
        self.assertTrue(isinstance(self.plain_text, str), "Error: plain text is not in str format")
        self.assertEqual(
            self.plain_text, self.password, "plain text and original password do not match"
        )
